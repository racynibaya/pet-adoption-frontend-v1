...
export default function StaffPets() {
  ...
  const confirmPet =
    confirmId != null ? visiblePets.find((p) => p.id === confirmId) : null;

  function toggleStatus(id: number, current: 'AVAILABLE' | 'PENDING') {
    updatePet(id, {
      status: current === 'AVAILABLE' ? 'PENDING' : 'AVAILABLE',
    });
  }
      ...
      <div className='staff-page-header'>
        <div>
          ...
          <p className='staff-page-sub'>
            {visiblePets.length} total ·{' '}
            {visiblePets.filter((p) => p.status === 'AVAILABLE').length} available
          </p>
        </div>
        <Link
          to='/staff/pets/add'
