...
const GALLERY_PHOTOS = [
  {
    ...
    svg: (
      <svg viewBox='0 0 400 600' width='100%' height='100%'>
        ...
        <path d='M192 392 Q200 404 208 392' stroke='#1d2235' strokeWidth='2' fill='none' />
      </svg>
    ),
  },
  {
  ...
  },
  {
    ...
    svg: (
      <svg viewBox='0 0 300 220' width='100%' height='100%'>
        ...
        <text x='150' y='120' textAnchor='middle' fontFamily='Fraunces, serif' fontWeight='700' fontSize='42' fill='#fff'>12k+</text>
        <text x='150' y='150' textAnchor='middle' fontFamily='Inter, sans-serif' fontWeight='500' fontSize='14' fill='#fff' opacity='0.9'>adoptions matched</text>
      </svg>
    ),
  },
  {
...
const VALUES = [
  {
    bg: 'var(--mint)', title: 'Animal first, always',
  ...
  {
    bg: 'var(--rose)', title: 'Warmth over forms',
  ...
  {
    bg: 'var(--sky)', title: 'Transparent process',
  ...
  {
    bg: 'var(--sun)', title: 'Diversity of homes',
  ...
  {
    bg: 'var(--cream)', title: 'Open communication',
  ...
  {
    bg: '#f4eaff', title: 'Proof of care required',
...
const TIMELINE = [
  { yr: '2023', title: 'Founded in Philippines', desc: 'Racyn adopts Eli. KodaNest launches as a sitter directory in 4 zip codes.' },
  { yr: '2024', title: 'Adoption matching', desc: 'Partnered with 50 shelters. First 1,000 adoptions matched. Series A led by Lerer Hippeau.' },
  { yr: '2025', title: 'Medi-care launches', desc: 'Partnered with 200+ vet clinics for routine and emergency care. Live cams roll out.' },
  { yr: '2026', title: '40 cities, 12k matches', desc: 'Now operating in 40 metros. Multi-pet plans, training, and grooming open to all members.' },
];

const TEAM = [
  {
    name: 'Racyn Ibaya', role: 'Founder & CEO', bg: '#f3d2b3',
    svg: (
  ...
  {
    name: 'Triccie Ann Rafin', role: 'Head of Adoption', bg: '#cfe6f7',
    svg: (
      <svg viewBox='0 0 200 200' width='100%' height='100%'>
  ...
  {
    name: 'Dr. Rowel Nepomuceno', role: 'Chief Veterinarian', bg: '#ffe9b8',
    svg: (
      <svg viewBox='0 0 200 200' width='100%' height='100%'>
  ...
  {
    name: 'Archie Alviar', role: 'Head of Training', bg: 'var(--mint)',
    svg: (
      <svg viewBox='0 0 200 200' width='100%' height='100%'>
...
export default function AboutPage() {
  return (
    <>
      ...
      >
        <span className='absolute top-[60px] left-[80px]'><PawprintDeco /></span>
        <span className='absolute bottom-[60px] right-[80px]'><PawprintDeco flip /></span>
        <Eyebrow style={{ justifyContent: 'center' }}>About KodaNest</Eyebrow>
        <h1 className='mt-16 max-w-[880px] mx-auto' style={{ fontSize: 80, lineHeight: 1.02 }}>
          Built by people<br />who can't stop<br />looking at dogs
        </h1>
        <p className='max-w-[620px] mx-auto text-[18px]' style={{ marginTop: 24, color: 'var(--ink-2)' }}>
          ...
          adoptable pets from verified shelters across 40 cities in the Philippines.
        </p>
      </section>
      ...
      <section style={{ padding: '0 0 64px' }}>
        <div
          className='grid gap-4 relative z-[5]'
          style={{ gridTemplateColumns: '1.3fr 1fr 1fr', marginTop: -48 }}
        >
          {GALLERY_PHOTOS.map(({ bg, svg }, i) => (
            <div
              key={i}
              className='photo transition-[transform,box-shadow] duration-[340ms] hover:-translate-y-[5px] hover:[box-shadow:0_14px_36px_rgba(18,52,64,0.14)]'
              style={{
      ...
      <section className='section'>
        <div className='grid gap-20 items-start' style={{ gridTemplateColumns: '1fr 1.2fr' }}>
          <div>
            ...
            <h2 className='mt-16' style={{ fontSize: 48, lineHeight: 1.08 }}>
              From one rescued<br />terrier to<br />twelve thousand<br />matches
            </h2>
          </div>
      ...
      <section style={{ padding: '32px 0 96px' }}>
        <div
          className='grid rounded-[28px] text-white'
          style={{
        ...
        >
          {ABOUT_STATS.map(({ num, lab }) => (
            <div key={lab} className='text-left'>
              <div className='text-white text-[48px] font-semibold [letter-spacing:-0.02em]' style={{ fontFamily: 'var(--font-display)' }}>{num}</div>
              <div className='text-[14px] mt-2' style={{ color: '#b8cec0' }}>{lab}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className='section-tight'>
        <SectionHead eyebrow='What we believe' heading="Six things we won't compromise on" />
        <div className='grid gap-6' style={{ gridTemplateColumns: 'repeat(3,1fr)' }}>
          {VALUES.map(({ bg, title, desc, icon }) => (
            <div
              key={title}
              className='bg-[var(--canvas)] border border-[var(--hairline-soft)] rounded-[16px] flex flex-row gap-[14px] items-start transition-[transform,box-shadow] duration-200 hover:-translate-y-[3px] hover:[box-shadow:var(--shadow-lift)]'
              style={{ padding: '16px 18px' }}
            >
              <div className='w-9 h-9 flex-shrink-0 rounded-[10px] flex items-center justify-center mt-[2px]' style={{ background: bg }}>{icon}</div>
      ...
      <section className='section-tight'>
        ...
        <div className='grid gap-6 relative timeline-line' style={{ gridTemplateColumns: 'repeat(4,1fr)' }}>
          {TIMELINE.map(({ yr, title, desc }) => (
            <div key={yr} className='relative pt-[60px] group'>
              <span className='tl-dot' />
              <div className='text-[24px] font-semibold mb-[6px]' style={{ fontFamily: 'var(--font-display)', color: 'var(--ink)' }}>{yr}</div>
              <h4>{title}</h4>
      ...
      <section className='section-tight'>
        ...
        <div className='grid gap-6' style={{ gridTemplateColumns: 'repeat(4,1fr)' }}>
          {TEAM.map(({ name, role, bg, svg }) => (
            <div key={name} className='text-center transition-transform duration-200 hover:-translate-y-[5px] group'>
              <div
                className='rounded-[20px] overflow-hidden mb-[14px] transition-[box-shadow] duration-200 group-hover:[box-shadow:0_12px_32px_rgba(18,52,64,0.14)]'
                style={{ aspectRatio: '1/1', background: bg }}
              >
                {svg}
              </div>
              <h4 style={{ fontSize: 17 }}>{name}</h4>
              <div style={{ color: 'var(--muted)', fontSize: 13, marginTop: 2 }}>{role}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className='section-tight'>
        <div className='cta-band'>
          <div>
            ...
            <p>We're hiring counselors, vets, and engineers in San Agustin HQ. Bring your pet to the interview.</p>
          </div>
          <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
            <Link to='/contact' className='btn btn-primary btn-lg'>Open roles</Link>
          </div>
        </div>
      </section>
    </>
